...
// 6. make sure your poisition is correct with fseek and use fwrite to update the record
...
FILE* updateTheFile(FILE* fPtr)
{
	...
	if(cust.accountNumber != 0 && accountNumber==cust.accountNumber){
        ...
        scanf("%s", cust.name);
        printf("old last name is %s what is the new lastname: ", cust.lName);
        scanf("%s", cust.lName);
        printf("old balance is %3f what is the new balance: ", cust.balance);
        scanf("%3f", &cust.balance);
